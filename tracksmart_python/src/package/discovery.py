...
BECKN_GATEWAY_API_KEY = os.getenv("BECKN_GATEWAY_API_KEY")

if not BECKN_GATEWAY_URL:
    raise RuntimeError("Environment variable BECKN_GATEWAY_URL is not set")
if not BECKN_GATEWAY_API_KEY:
    raise RuntimeError("Environment variable BECKN_GATEWAY_API_KEY is not set")
...
def discover(request_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    headers = {
        ...
        "x-api-key": BECKN_GATEWAY_API_KEY
    }
